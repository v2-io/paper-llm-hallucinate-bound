...
>
> $\mathbb{E}\,d_{FR}\bigl(P_{M_{\tau^+}\mid e, M_{\tau^-}, G},\, P_{M_{\tau^+}\mid e, M_{\tau^-}}\bigr) \;\leq\; (\pi/\sqrt 2) \cdot \sqrt{I(G;\,M_{\tau^+}\mid e_\tau, M_{\tau^-})}$.
>
> The constant $\pi/\sqrt{2} \approx 2.22$ is universal, dimension-free, global — no small-information condition required, and (R), (K) are not invoked.

> [!proof]
> On the unit $L^2$-sphere of $\sqrt{p}$, the chord-arc identity ([[#^lem-chord-arc]]) gives $\mathrm{Hel}(P, Q) = \sqrt{2}\sin(d_{FR}(P, Q)/4)$ exactly. [[#^lem-chord-arc]] shows $d_{FR}(P, Q) \le \pi\,\mathrm{Hel}(P, Q)$ globally, with equality at the antipode $\mathrm{Hel} = 1$. By \citealt[Lemma 2.4]{tsybakov-2009-nonparametric}, $2\,\mathrm{Hel}^2 \le \mathrm{KL}$ slice-wise. Combining: $d_{FR}^2 \le \pi^2\mathrm{Hel}^2 \le (\pi^2/2)\,\mathrm{KL}$ slice-wise. Take $\mathbb{E}_G$, substitute the chain rule [[#^lem-chain-rule]]: $\mathbb{E}\,d_{FR}^2 \le (\pi^2/2)\,I_M$. Jensen on $\sqrt{x}$: $\mathbb{E}\,d_{FR} \le (\pi/\sqrt{2})\sqrt{I_M}$. $\square$

The $\pi/2 \approx 1.57$ overhead vs. the locally-tight $\sqrt{2}$ is exactly the worst-case arc-chord ratio at the unit $L^2$-sphere's antipode — a geometric maximum, not a slack. Equality is achieved in the limit $\mathrm{Hel} \to 1$ at the antipode, when goals can drive the post-update measure to a near-disjoint support from the marginal.
...
The three Fisher-Rao-spine bounds — locally-tight $\sqrt{2}$ (under (H1)+(H4$'$)+(PI)+(R)+(K)), globally-valid $\pi/\sqrt{2}$ ([[#^thm-track2-global]], under (H1)+(PI) only), and Hellinger chord $1/\sqrt{2}$ ([[#^thm-hellinger]], under (H1)+(PI) only) — share the same intrinsic geometry and have strictly nested hypotheses. There is no "metric switch" between regimes: macroscopic hallucination is in Fisher-Rao directly via [[#^thm-track2-global]]; [[#^thm-hellinger]] is the chord companion on the same unit $L^2$-sphere.
