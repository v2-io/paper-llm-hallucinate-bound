...
Outside the uniform-locality regime — when a few rare goals carry disproportionately large transferred information, or when the operating distribution sits at moderate-to-large $I_M$ — the slice-wise expansion's $(1+o(1))$ remainder is no longer uniformly sharp. A global *ambient spherical-arc* bound is available under strictly weaker hypotheses, at the cost of a slightly larger constant. (The bound is on the Bhattacharyya / Fisher spherical-arc pseudometric $d_{FR} = 2\arccos(\mathrm{BC})$ on the unit $L^2$-sphere of $\sqrt p$, which equals intrinsic Fisher-Rao only on the ambient simplex / full nonparametric distribution space; on parametric submanifolds it is the chart-invariant pseudometric, $\le$ submanifold-intrinsic geodesics.)

> [!theorem] Track 2 global Fisher-Rao spherical-arc backstop ^thm-track2-global
...
> As in [[#^sec-conjugate-gauss-numerics]], the agent uses the conjugate Gaussian prior/likelihood update rule ($\theta \sim \mathcal{N}(0,\tau^2)$, $\Omega \mid \theta \sim \mathcal{N}(\theta, \sigma^2)$), while the operating distribution probing goal-coupling has $\Omega \mid G = g \sim \mathcal{N}(\beta(g), \sigma^2)$. The post-update parameter law under operating $G$ is the Gaussian $\mu_+ \mid G \sim \mathcal{N}(L_{\text{post}}\beta(G),\, L_{\text{post}}^2\sigma^2)$ with $L_{\text{post}} = \tau^2/(\sigma^2+\tau^2)$. The locally-tight Fisher-Rao bound translates to the Euclidean bound on random-parameter displacement
>
