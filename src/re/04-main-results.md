...
*Track 1 generalizes the Stuart-school cascade.* Under Stuart-school Lipschitz-posterior hypotheses \cite{stuart-2010-acta,sprungk-2020-local-lipschitz,dolera-mainini-2023-aihp-lipschitz,hosseini-hsu-taghvaei-2024-conditional-ot} plus $T_2$ concentration on the goal-induced parameter measure and slice-wise conditional sub-Gaussianity, [[#^thm-stuart-school-reduction]] yields the canonical $C_{T_2} \propto L_{\text{post}}^2/\rho_{\text{noise}}$ — tight at $2L_{\text{post}}^2/\rho_{\text{noise}}$ on conjugate-Gaussian and exponential-family-conjugate instances ([[#^sec-conjugate-gauss-numerics]]); Track 1 extends the cascade beyond Class 1 (Separated) to goal-coupled architectures, a regime Stuart-school does not address.
...
> Under (H1)+(PI)+(R)+(K)+(H4$'$), in the umbrella form [[#^eq-umbrella]] (where $\|\Delta M_{\text{bias}}\|$ is the $d_{\mathcal{M}}$-displacement per [[#^eq-bias-quantity]]): *(a)* $d_{\mathcal{M}}$ is uniquely the ambient Amari-Nagaoka Fisher-Rao spherical-arc distance, agreeing with submanifold-intrinsic Fisher-Rao geodesics at second order in the (H4$'$) regime; *(b)* $C = \sqrt 2$ is the unique sharp upper-bound constant — no $C < \sqrt 2$ holds uniformly across the (H4$'$) regime.
...
*Transferred information has a closed-form empirical estimator.* For $G \in \{g_1, g_2\}$ uniform, $I(G;\,M_{\tau^+}\mid e_\tau, M_{\tau^-}) = \mathrm{JSD}(P_{M_{\tau^+}\mid g_1},\,P_{M_{\tau^+}\mid g_2})$ — Monte-Carlo estimable from per-goal response samples; the local Track-2 form gives $\mathbb{E}\,\|\Delta M_{\text{bias}}\| \le \sqrt{2\,\widehat{\mathrm{JSD}}}$, the global form $\mathbb{E}\,d_{FR} \le 2\sqrt{\widehat{\mathrm{JSD}}}$ ([[#^sec-track2-companions]]). Factorization additionally requires lower-bounding $I(G;\Omega\mid e, M_{\tau^-})$ — architecture-side modeling, [[#^sec-conclusion]].
