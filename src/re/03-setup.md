...
*Coupled-class autoregressive connectivity (deferred).* [[#^lem-attention-coupled]] establishes that plain decoder-only transformer attention — and the broader autoregressive-sequence-model family with causally-aggregated cross-position mixers under a per-source non-degeneracy condition (linear attention, Mamba/SSMs, RWKV, RetNet, long-convolutions; named-architecture verifications + ICL-correspondence caveat in [[#^sec-attention-coupled-proof]]) — is structurally Class 3 by directed-graph reachability. The claim is on $\kappa_{\text{processing}}$'s *support*, not its magnitude.
