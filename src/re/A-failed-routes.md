...
**Attempt.** Treat $G$ as a parameter to be estimated from $\Omega$ given $(e, M)$. The Cramér-Rao lower bound gives $\mathrm{Var}(\hat{G}) \geq \mathbf{I}_G^{-1}$ where $\mathbf{I}_G$ is the Fisher information about $G$ given $(\Omega, e, M)$. Invert this to a posterior-sensitivity factor: an upper bound $\|\Delta M_{\text{bias}}\| \leq L_M \cdot \sqrt{\mathbf{I}_G^{-1}}$ via a Lipschitz-in-$G$ constant on the coupled-update mechanism.
