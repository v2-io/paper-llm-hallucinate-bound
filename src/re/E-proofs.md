...
>
> Under (H4$'$) the remainder is controlled uniformly: with bounded Amari-Chentsov tensor (\citealt{amari-nagaoka-2000-info-geom} §5) $|T_{ijk}\delta^i\delta^j\delta^k| \le \mathcal{T}_\star\|\delta\|_{\text{coord}}^3$ on the goal-induced neighborhood and minimum Fisher eigenvalue $\mathbf{I}_{\min} > 0$, the slice-wise inequality $d_{FR}^2(P_{M_{\tau^+}\mid G=g}, P_{M_{\tau^+}}) \le 2\,\mathrm{KL}_g(1 + R_3(\delta_\star))$ holds with $R_3(\delta_\star) = (\mathcal{T}_\star/3)\sqrt{2\delta_\star/\mathbf{I}_{\min}^3} \to 0$ as $\delta_\star \to 0$. *Derivation:* the Lagrange-form Taylor remainder gives $|\mathrm{KL} - \tfrac{1}{2}d_{FR}^2| \le (\mathcal T_\star/6)\|\delta\|_{\text{coord}}^3$. Since $d_{FR}^2 = \delta^\top \mathbf I\,\delta \ge \mathbf I_{\min}\|\delta\|_{\text{coord}}^2$, we get $\|\delta\|_{\text{coord}} \le d_{FR}/\sqrt{\mathbf I_{\min}}$, hence $\|\delta\|_{\text{coord}}^3 \le d_{FR}^3/\mathbf I_{\min}^{3/2}$. Substituting and rearranging with $d_{FR} \approx \sqrt{2\mathrm{KL}_g}$ at leading order yields $d_{FR}^2 \le 2\mathrm{KL}_g(1 + (\mathcal T_\star/3)\sqrt{2\mathrm{KL}_g/\mathbf I_{\min}^3})$, and $\mathrm{KL}_g \le \delta_\star$ under (H4$'$) gives the stated $R_3$.
>
