...
*Outside (H4$'$).* When rare goals carry disproportionately large transferred information, the slice-wise expansion is no longer uniformly sharp. Track 2 has globally-valid companion bounds — ambient Fisher-Rao spherical-arc at $C = 2$ via Rényi-1/2 monotonicity + exact chord-arc identity, and a Hellinger chord at $1/\sqrt{2}$ — under (H1)+(PI) only, full taxonomy in [[#^sec-track2-companions]]. The $\sqrt 2$ overhead vs. the locally-tight $\sqrt 2$ is the chord-arc factor at the unit $L^2$-sphere's antipode in squared-distance form — a geometric maximum, not a slack.

### No-go — chart rescaling ^sec-no-go-mechanism

A chart rescaling $\phi \mapsto a\phi$ scales the chart-Euclidean $W_2$ linearly while leaving KL, conditional MI, Fisher-Rao spherical-arc, and Hellinger chart-invariant ($f$-divergences and intrinsic-geometry quantities are reparameterization-invariant). For any architecture realizing finite positive $W_2^\phi$ and finite positive $I$ — automatic on any non-trivial goal-coupled architecture — taking $a \to \infty$ contradicts a candidate fixed $C_0\sqrt I$. The Gaussian scale family with charts $\sigma$ vs. $\log\sigma$ is the canonical illustration. The implication is constructive: a universal constant requires a coordinate-invariant norm; Fisher-Rao plays this role under (PI)+(R)+(K) via Čencov uniqueness. Full proof in [[#^sec-no-go-proof]].
