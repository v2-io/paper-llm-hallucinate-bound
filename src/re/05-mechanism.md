...
The implication is constructive. To get a universal constant the bound must be stated in a coordinate-invariant norm — one that respects the underlying geometry of the statistical manifold rather than an arbitrary chart on it. Fisher-Rao plays this role under (PI)+(R)+(K), via Čencov uniqueness ([[#^thm-fr-uniqueness]]). Alternative chart-independent commitments (TV with Pinsker, Hellinger-as-divergence, $\chi^2$, Rényi) yield strictly weaker bounds — the no-go narrows the candidate space to coordinate-invariant metrics, and the (PI)+(R)+(K) triple then selects Fisher-Rao + $\sqrt 2$.
