...
This classification is the *Pearl-blanket form* of the architectural-separation distinction. It is structurally consistent with — but does not adopt — the Friston-blanket reading \cite{friston-2013,friston-2019} in which Markov blankets are taken to demarcate self-from-other in a metaphysically substantive way. Bruineberg et al. argue that the Friston-blanket reading overruns what the conditional-independence formalism delivers; we agree, and the explicit Coupled-class scope-exit (plain decoder-only transformer inference does not satisfy architectural separation under the token-distribution-as-belief-state idealization) is the scope honesty their critique calls for.
